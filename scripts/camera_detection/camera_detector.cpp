...
CameraObjectDetector::CameraObjectDetector(std::string modelWeights, std::string modelConfig,
                                           std::string classesFilename, float confThresh, float nmsThresh)
...
void CameraObjectDetector::detectObject(cv::Mat& frame, std::vector<cv::Mat>& info)
{
  ...
  // Scan through all the bounding boxes and keep only the ones with high
  // confidence
...
void CameraObjectDetector::drawDetections(cv::Mat& image, std::vector<cv::Mat>& info)
{
  for (const auto& detection : info)
  {
    ...
    cv::putText(image, label, cv::Point(left, top - labelSize.height), cv::FONT_HERSHEY_SIMPLEX, 0.5,
