...
bool compareBoxes(const BBox& a, const BBox& b, const float displacement_thresh, const float iou_thresh)
{
...
std::vector<std::vector<int>> associateBoxes(const std::vector<BBox>& prev_boxes, const std::vector<BBox>& curr_boxes,
...
std::vector<std::vector<int>> connectionMatrix(const std::vector<std::vector<int>>& connection_pairs,
                                               std::vector<int>* left, std::vector<int>* right)
{
  for (auto& pair : connection_pairs)
  {
    const bool left_found = std::any_of(left->begin(), left->end(), [pair](int i) { return i == pair[0]; });
    if (!left_found)
      left->push_back(pair[0]);
    const bool right_found = std::any_of(right->begin(), right->end(), [pair](int j) { return j == pair[1]; });
    if (!right_found)
...
bool hungarianFind(const int i, const std::vector<std::vector<int>>& connection_matrix,
...
int searchBoxIndex(const std::vector<BBox>& boxes, const int id)
{
